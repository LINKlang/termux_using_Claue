...
\033[0;37m选项e 修改 Superfetch\033[0m
\033[0;33m--------------------------------------\033[0m
...
"
    ...
    8)
        ...
        if [ $PreventImperson_choice == "Y" ] || [ $PreventImperson_choice == "y" ]; then
            if [ $PreventImperson_value == 'false' ]; then
                #将false替换为true
            ...
            elif [ $PreventImperson_value == 'true' ]; then
                #将true替换为false
    ...
    e)
        # 修改 Superfetch
        Superfetch_value=$(grep -o '"Superfetch": [^,]*' $clewd_dir/config.js | cut -d' ' -f2)
        echo -e "当前Superfetch值为\033[0;33m $Superfetch_value \033[0m"
...
\033[0;37m选项2 测试版\033[0m"
            while :; do
                read -n 1 stupdate
                [ "$stupdate" = 1 ] && {
                    git clone https://ghproxy.net/https://github.com/SillyTavern/SillyTavern.git SillyTavern_new
                    break
                }
                [ "$stupdate" = 2 ] && {
                    git clone -b staging https://ghproxy.net/https://github.com/SillyTavern/SillyTavern.git SillyTavern_new
                    break
                }
    ...
    5)
        ...
        if [[ "$cuschoice" == [yY] ]]; then
            ...
            grep -q '<optgroup label="自定义">' "SillyTavern/public/index.html" && sed -i "/<optgroup label=\"自定义\">/a\ \ \ \ <option value=\"$CUSTOM_INPUT_VALUE\">$CUSTOM_INPUT_VALUE</option>" "SillyTavern/public/index.html" || {
                sed -i "/<optgroup label=\"GPT-3.5 Turbo\">/i\<optgroup label=\"自定义\">\n\ \ \ \ <option value=\"$CUSTOM_INPUT_VALUE\">$CUSTOM_INPUT_VALUE</option>\n</optgroup>" "SillyTavern/public/index.html"
                sed -i "/<optgroup label=\"Versions\">/i\<optgroup label=\"自定义\">\n\ \ \ \ <option value=\"$CUSTOM_INPUT_VALUE\">$CUSTOM_INPUT_VALUE</option>\n</optgroup>" "SillyTavern/public/index.html"
            }
    ...
    7)
        ...
        [[ "$delSTChoice" == [yY] ]] && {
            echo -e "开始删除喵~"
            ls | grep "^SillyTavern_\([^o].*\|..+\.?.*\)$" | xargs -d"\n" rm -r
            echo -e "旧版本酒馆删除完成了喵~"
        } || echo "什么都没有执行喵~" >&2
        ;;
    8)
...
function TavernAI-extrasinstall {
    ...
    if [ ! -d "/root/TavernAI-extras" ]; then
        ...
        [ -d /root/TavernAI-extras ] || {
            echo "TavernAI-extras（酒馆拓展）安装失败，请更换网络后重试喵~"
            exit 8
        }
    fi
    ...
    [ "$requirementschoice" = "y" ] || [ "$requirementschoice" = "Y" ] && {
...
function TavernAI-extrasstart {
    ...
    print_selected() {
        ...
        echo "喵喵当前选择了: $(
            IFS=,
            echo -e "\033[0;36m${selected_modules[*]}\033[0m"
        )"
    }
